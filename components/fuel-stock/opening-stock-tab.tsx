...
// Opening stock tab with form to set opening stock per fuel type
...
export function OpeningStockTab() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/fuel-stock/opening', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...formData,
          openingStockLiters: parseFloat(formData.openingStockLiters),
        }),
      })
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Set Opening Stock</CardTitle>
        <CardDescription>Record opening stock levels for each fuel type</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="opening-stock">Opening Stock (Liters)</Label>
              <Input
                id="opening-stock"
                type="number"
                step="0.01"
                min="0"
                value={formData.openingStockLiters}
                onChange={(e) => setFormData({ ...formData, openingStockLiters: e.target.value })}
                required
                disabled={loading}
                placeholder="0.00"
              />
